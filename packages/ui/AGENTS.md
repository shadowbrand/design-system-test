...
| #U2 | 🔵 | `Card` — self-contained tile (`<article>`), fixed layout title → body → CTA (the CTA is a `Button`); props (all optional): `title`, `body`, `cta`, `onCtaClick` | dist/index.d.ts | ~200 |
| #U6 | 🔵 | `Button` — tokenized `<button>`, body type; props: `children` (label), `onClick`, `disabled` (muted, no hover), `type` (default `"button"`) | dist/index.d.ts | ~200 |
