...
| #U2 | 🔵 | `Card` — self-contained tile (`<article>`), fixed layout title → body → CTA (the CTA is a `Button`); props (all optional): `title`, `body`, `cta`, `onCtaClick` | dist/Card.d.ts | ~100 |
| #U6 | 🔵 | `Button` — tokenized `<button>`, body type; props: `children` (label), `onClick`, `disabled` (default false; muted, no hover), `type` (default `"button"`) | dist/Button.d.ts | ~100 |
